...
export const addCar = expressAsyncHandler(async (req, res) => {
    ...
    newCar.save().then((car) => {
        ...
        if (err.code === 11000) {
            Car.findOne({ carId: req.body.carId, isDeleted: true }).then((car) => {
                if (car) {
                    car.isDeleted = false;
...
export const bulkCarUpload = expressAsyncHandler(async (req, res) => {
    ...
    form.parse(req, async (err, fields, files) => {
        ...
        csv().fromFile(files.file.filepath).then((cars) => {
            for (let i = 0; i < cars.length; i++) {
                ...
                newCar.save().catch(err => {
                    if (err.code === 11000) {
                        ...
                        Car.findOne({ carId: cars[i].carId, isDeleted: true }).then((car) => {
                            if (car) {
                                car.isDeleted = false
                                car.save().then((result) => {
